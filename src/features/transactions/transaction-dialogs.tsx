  ...
  const submit = async () => {
    ...
    if (items.length === 0) {
      toast.error("Keranjang kosong", {
        description: "Gunakan Void jika ingin membatalkan transaksi sepenuhnya.",
      })
      return
    }
    setSubmitting(true)
    try {
      const response = await createCorrection(
        session,
        props.transaction.offlineUuid || props.transaction.id,
        {
          reason: reason.trim(),
      ...
      const newOfflineUuid =
        response.data?.data?.id_new_transaction || crypto.randomUUID()
      ...
      <DialogContent className="max-w-md">
          ...
          <div className="grid gap-4">
            <div className="flex justify-between rounded-md bg-secondary p-3 text-xs">
              <span className="font-mono text-muted-foreground">
                {props.transaction.invoiceNumber || props.transaction.id}
              </span>
        ...
        <DialogFooter className="flex flex-col items-center justify-end gap-2 sm:flex-row">
          {reason.length > 0 && reason.length < 10 && (
            <span className="mr-auto flex-1 text-[10px] text-destructive">
          ...
          <Button
            type="button"
            disabled={reason.length < 10 || items.length === 0 || submitting}
            onClick={() => void submit()}
          >
            Kirim Koreksi
          </Button>
    ...
    <Dialog open={props.open} onOpenChange={props.onOpenChange}>
      <DialogContent>
        ...
        <DialogFooter>
          ...
          <Button
            variant="destructive"
            disabled={reason.length < 8 || submitting}
            onClick={() => void submit()}
          >
            Buka Kasus
          </Button>
